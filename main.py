...
class RateLimiter:
    ...
    # ⚠️ Внимание! метод .pipeline() не исполняет операцию атомарно!
    # Используйте метод `is_limited()`, в котором используется lua скрипт, запускающийся атомарно
    async def is_limited_video(
            self,
            ip_address: str,
            endpoint: str,
            max_requests: int,
    ...
    # ✅ Корректная реализация с lua скриптом
    async def is_limited(
        ...
        await self._load_script()  # Загружаем lua скрипт
